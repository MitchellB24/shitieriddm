...
// This sketch works with only 1 servo, to run the sketch without the adafruit shield
Servo arduinoServo;
...
void setup() {
  ...
  arduinoServo.attach(9,500,2500);
  ...
  for (int i = 0; i < numServo; i++) {
    if (control_PWM) {
      ...
      for (int j = USMIN; j < USMAX; j++) {
        arduinoServo.writeMicroseconds(j);
      }
      delay(delay_time);
